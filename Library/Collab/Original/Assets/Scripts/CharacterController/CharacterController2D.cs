...
public class CharacterController2D : MonoBehaviour
{
    ...
    public bool CanJumpAttack { get { return !state.IsGrounded && !state.IsClimbing; } }
    ...
    void FixedUpdate()
    {
        ...
        bool climbingWallWasAtRightSide = state.ClimbingWallAtRightSide;
        ...
        for (int i = 0; i < 2; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                ...
                if (raycastHit && raycastHit.collider.gameObject.CompareTag("Climbeable"))
                {
                    ...
                    state.ClimbingWallAtRightSide = rightside;
                    break;
                }
            }
        }
    ...
    {
        Debug.Log("SET VFORCE = " + force.ToString());
        _velocity.y = force;
    }

    public void AddVerticalForce(float force)
    {
        Debug.Log("ADD VFORCE = " + force.ToString());
        _velocity.y += force;
    }
    ...
    public void ClimbUpDown(bool up)
    {
        float climbMagnitude = up ? Parameters.climbMagnitude : -Parameters.climbMagnitude;
        SetVerticalForce(climbMagnitude);
    }
