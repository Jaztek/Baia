...
public class CharacterController2D : MonoBehaviour
{
        ...
        foreach (Intent intent in intents)
        {
            switch (intent.type)
            {
                ...
                case Intent.Type.JUMP:
                    ...
                    {
                        Jump();
                        animatorController.setAnimation(PlayerAnim.JUMP);
                    }
                    break;
                case Intent.Type.UNJUMP:
                    if (!CanJump())
                    {
                        UnJump();
                    }
                    break;
                case Intent.Type.ROLL:
                    AddRollVelocity(1);
                    //AddHorizontalVelocity(0);
                    break;
    ...
    void AddRollVelocity(int direction)
    {
        //float acceleration = state.isCollidingBelow ? Parameters.speedAccelerationOnGround : Parameters.speedAccelerationOnAir;
        animatorController.setAnimationRoll(PlayerAnim.ROLL);
